...
function LHAnimationFrameInfo:frameWithDictionary(frmInfo)
	...
	local dictNotif = frmInfo:dictForKey("notifications");
	
	--dictNotif:print();
	
	--print("KEY " .. tostring(dictNotif:stringForKey("FruitName")));
	
	--print("NUMBER OF KEYS " .. tostring( dictNotif:numberOfKeys() ));
	if(dictNotif and #dictNotif:allKeys() > 0)then
		object.lhNotifications = LHDictionary:initWithDictionary(dictNotif);
	end
...
function LHAnimationNode:enterFrame( event )
	...
	if(frmObject.lhDelayPerUnit*self.lhDelayPerUnit <= self.elapsedFrameTime)then
	    ...
	    --print("enter frame " .. self.lhUniqueName .. " time " .. tostring(event.time) .. " frm: " ..tostring(self.currentFrame));
		...
		if(activeFrame.lhNotifications)then
